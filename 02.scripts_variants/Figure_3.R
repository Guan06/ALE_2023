...
# ggplot(t1, aes(Compound_conc2, MIC80_OD)) +
#   geom_jitter(aes(color = Compound_conc2), alpha = 0.2,
#               width = 0.3, height = 0) +
#   scale_color_manual("Compound concentration", values = cc_color) +
#   geom_violin(fill = "NA") + 
#   main_theme
# 
# summary(aov(MIC80_OD ~ Compound_conc2, data = t1))
...
t2 <- t2 %>% mutate(Rep = case_when(
  ...
  (Date == "20240720" | Date == "20240705") ~ 'Bio_rep_1',
  (Date == "20240802" | Date == "20240808") ~ 'Bio_rep_2',
))
...
ggsave("../04.figures/Figure_3.pdf", width = 12, height = 7)

...
See details about this in next session (__Figure 2d__).
...
As we shown above, if we filter out variants with QUAL < 1, we will have the following variants remain:

```{r filter_q, echo=FALSE, message=FALSE, error=FALSE}
 kbl(this_gene %>% filter(QUAL > 1) %>% select(Sample_ID, POS, EVIDENCE, QUAL, EFFECT,
                                               Compound, Concentration)) %>%
      kable_styling(full_width = F)
```

Alternatively, we could choose to filter out the variants that have < 5 reads support:

```{r filter_depth, echo=FALSE, message=FALSE, error=FALSE}
 kbl(this_gene %>% filter(Alt_depth > 4) %>% select(Sample_ID, POS, EVIDENCE, QUAL, EFFECT,
                                               Compound, Concentration)) %>%
      kable_styling(full_width = F)
```

## Alternative supporting reads number (Alt_depth)

Since filter by QUAL introduced false negative, we tried another filtering, will be referred 
as _re-filter-2_.

```{r filter_by_alt_depth, echo=FALSE, message=FALSE, error=FALSE, fig.width=6, fig.height=2}
  mermaid("
  graph LR;
      A[All variants] --> B[Alt_depth >= 5];
      B --> C[Allele frequency > 0.05];
      C --> D[Filter out 6 parental variants]
  ")
```
...
As we can see, variants with low AF also have very bad quality. So we will filter out variants with AF > 0.5 as the cutoff for parental variants here (namely at the following positions `r unique(v2025_filter5_parental[v2025_filter5_parental$Ratio > 0.5, ]$POS)`.

This gives the same list of variants (except for _1914986_ which doesn't have good mapping results) as before, so same __six__ parental variants. 

### Remove parental variants

We then remove the parental variants from the evolved populations and plotted the
quality vs allele frequency. As we can see from below, there's a clear cutoff of
quality at allele frequency 0.5. 
...
```{r filter5_evolved, echo=FALSE, message=FALSE, error=FALSE, fig.width=5, fig.height=3}
  ...
  v2025_filter5_evolved <- v2025_filter5_evolved[!(v2025_filter5_evolved$POS %in% parental_lst), ]
# 
...
We will check the compound prevalence of variants, i.e. how many different compound ()
